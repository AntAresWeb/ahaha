...
class PostgresAnalysisRepository(AnalysisRepository):
    ...
    async def save(self, analysis: Analysis) -> Analysis:
        ...
        update_data = analysis_to_orm_dict(analysis)

        stmt = (
            update(AnalysisORM)
            .where(AnalysisORM.id == analysis.id)
            .values(**update_data)
        ...
        if orm_analysis is None:
            raise ValueError(f"Анализ с id {analysis.id} не найден")
    ...
    async def get_by_id(self, analysis_id: int) -> Analysis | None:
        ...
        stmt = select(AnalysisORM).where(AnalysisORM.id == analysis_id)
        result = await self._session.execute(stmt)
        orm_analysis = result.scalar_one_or_none()
        if orm_analysis is None:
            return None
